...
A 2D pixel-art incremental game. A raccoon inherits Grandmama's farm - one old hen, a bare field and a bicycle - and, being a raccoon, decides to get super rich. You are the hand that runs the company the raccoon founds: farms, factories, a crew, a fleet, a gene lab, a kitchen, a chicken park, branches on a spinning globe, a time machine and eventually a seat on the stock market, across twenty plots of a 1280x832 valley and six **ages** of the company, from straw to Jurassic. Every pixel is generated procedurally in code - the two pixel fonts included - so there are no image files, no emoji and no icon fonts.
...
Big, chunky and almost wordless. Five fat pills in the corner - coins, feathers, feed, chickens held out of capacity, and the age the company is in - and a row of oversized toolbelt keys with nothing but a picture on each. The feed pill goes red when the barn is empty; the capacity pill goes red when the coops are full, which is when hatching stops.
...
Nothing important is buried inside another screen. A **desk bar** runs along the bottom-left with one fat key each for the **Lab**, the **Index**, **Mama**, the **crew payroll**, the **depot**, the **world** and the **gene lab** - locked ones stay in place behind a padlock and say what unlocks them, so the row never shuffles under your finger. The tools sit opposite on their own leather strap, each with the number key that picks it printed on the corner, and the locked ones wear the same padlock. The camera may scroll past the strap, so the road, the bike, the company sign and the customers are never stuck behind the buttons; a new ranch opens looking at the road.
...
- UI icons are a set of 81 hand-authored 10x10 pixel glyphs rendered to canvases - the interface has no emoji at all.
