...
- The **Lab shack** (or the LAB key on the desk bar) puts you at a desk. There is a keyboard, a mug, sticky notes, and an old beige monitor that boots EGGOS and runs SKILLMAP.EXE: research as a **map of blocky hexagons** joined by paths, fanning out from the kernel in the middle along eight modules (crew, gather, hens, hatch, love, factory, market, farm), 79 packages in all. Every hex wears its branch's colour so the limbs read at a glance - state is carried by brightness instead, with affordable packages lit and ringed in pulsing gold and unaffordable ones sitting dark. Clusters are labelled with their branch name and the taskbar chips double as the colour key. A pixel mouse pointer follows your finger across the glass. Only hexes you have installed or could install right now are lit; the rest of the map is dark, with dotted stubs and question marks hinting where the paths go on. Gold hexes install with one click on the README window's button; module chips on the taskbar pan the map to each branch. Drop a chicken on the Lab to graduate it for bonus feathers.
...
Nothing important is buried inside another screen. A **desk bar** runs along the bottom-left with one fat key each for the **Lab**, the **Index**, **Mama**, the **crew payroll** and the **depot** - locked ones stay in place and say what unlocks them, so the row never shuffles under your finger. The tools sit opposite on their own leather strap, each with the number key that picks it printed on the corner.
...
| `js/ui.js` | title screen, camera renderer, the tool rack and the desk bar, the two-dock palettes, the terrain paint layer, pixel-cloud tooltips, the job-application crew forms, hatch animation, inspect, the crew board and robot workshop, the book, the HQ and its hand-drawn wall map, the trip window, the Lab desk and its hex map, particles, sound |
